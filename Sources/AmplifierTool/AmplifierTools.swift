...
let getSourcesTool = MCPTool<AmplifierInput, String>(
    ...
    body: { input async throws -> String in
        ...
        let jsonData = try JSONSerialization.data(withJSONObject: sourcesWithIndices, options: [.prettyPrinted, .sortedKeys])
...
let getStatusTool = MCPTool<AmplifierInput, String>(
    ...
    body: { input async throws -> String in
        ...
        // Convert to JSON string
        let jsonData = try JSONSerialization.data(withJSONObject: statusDict, options: [.prettyPrinted, .sortedKeys])
        return String(decoding: jsonData, as: UTF8.self)
    }
)
